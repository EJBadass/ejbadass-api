...
public class BClassFinder {
	...
	public static Class<?> find(Class<?> f) throws Exception {
		...
		if(f.isInterface()) {
			...
			if(nbPreferred==1) res = firstPrefered;
			else if(nbPreferred==0) {
				
				if(possibilities.size()==1) res = ((Class<?>)possibilities.iterator().next());
				
				else if(possibilities.size()==0) throw new NoImplementationFoundException();
				
				else throw new MultipleImplementationFoundException();
				
			} else throw new MultiplePreferedImplementationFoundException();
